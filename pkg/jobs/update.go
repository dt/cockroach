...
`
	...
	v, err := u.txn.GetSystemVersion(ctx)
	if err != nil {
		return err
	}
	if v != nil && v.AtLeast(clusterversion.V24_3_AddJobsTables.Version()) {
		if ju.md.Status != "" && ju.md.Status != status {
			if err := j.StatusStorage().Write(ctx, u.txn, JobStatusState, string(ju.md.Status)); err != nil {
				return err
			}
		}
		...
		if progress != nil {
			...
			if err := j.ProgressStorage().Write(ctx, u.txn, float64(progress.GetFractionCompleted()), ts); err != nil {
				return err
			}

			if progress.RunningStatus != beforeProgress.RunningStatus {
				if err := j.StatusStorage().Write(ctx, u.txn, JobStatusInfo, progress.RunningStatus); err != nil {
					return err
				}
			}

			if progress.TraceID != beforeProgress.TraceID {
				if err := j.StatusStorage().Write(ctx, u.txn, JobStatusTrace, fmt.Sprintf("%d", progress.TraceID)); err != nil {
					return err
				}
			} else {
				log.Infof(ctx, "%d trace before after %d, %d", j.id, md.Progress.TraceID, progress.TraceID)
			}
		}
	}
