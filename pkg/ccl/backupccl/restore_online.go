...
func sendAddRemoteSSTs(
	...
	for i := 0; i < restoreWorkers; i++ {
		grp.GoCtx(sendAddRemoteSSTWorker(execCtx, dataToRestore.getRekeys(), dataToRestore.getTenantRekeys(), restoreSpanEntriesCh, requestFinishedCh))
	}
...
func sendAddRemoteSSTWorker(
	execCtx sql.JobExecContext,
	tableRekeys []execinfrapb.TableRekey, tenantRekeys []execinfrapb.TenantRekey,
	...
	return func(ctx context.Context) error {
		...
		for _, i := range tableRekeys {
			...
			if err := protoutil.Unmarshal(i.NewDesc, &desc); err != nil {
				return errors.Wrapf(err, "unmarshalling rekey descriptor for old table id %d", i.OldID)
			}
			table, _, _, _, _ := descpb.GetDescriptors(&desc)
			if table == nil {
				return errors.New("expected a table descriptor")
			}
