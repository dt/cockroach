...
func TestMergeSubscriptionsRun(t *testing.T) {
	...
	t.Run("returns without close when all all events are consumed", func(t *testing.T) {
		sub1, err := mockClient.Subscribe(ctx, 0, 0, streamclient.SubscriptionToken("partition1"), hlc.Timestamp{}, hlc.Timestamp{}, nil)
		require.NoError(t, err)
		sub2, err := mockClient.Subscribe(ctx, 0, 0, streamclient.SubscriptionToken("partition2"), hlc.Timestamp{}, hlc.Timestamp{}, nil)
		require.NoError(t, err)
