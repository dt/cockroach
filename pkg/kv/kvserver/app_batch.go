...
func (b *appBatch) runPostAddTriggers(
	...
	// CloneData runs in runPostAddTriggersReplicaOnly so it has access to
	// the dst replica (needed for the InconsistentReplicas-bit check that
	// guards against replay-after-unlock, and for the missing-spans
	// bookkeeping the unlock-time completeness check consumes).
