...
// LRU cache and asynchronously persists new entries into
...
type StatementStore struct {
	...
	memAcc       *mon.ConcurrentBoundAccount
...
func NewStatementStore(s *cluster.Settings, knob *TestingKnobs) *StatementStore {
	...
	ss.cacheMu.cache = cache.NewUnorderedCache(cache.Config{
		...
		OnEvicted: func(key, value interface{}) {
			if ss.memAcc != nil {
				ss.memAcc.Shrink(context.Background(), cacheEntrySize)
			}
		},
	})
	return ss
...
func (ss *StatementStore) PutStatement(ctx context.Context, info StatementInfo) {
	...
	ss.appendPending(ctx, info)
}

func (ss *StatementStore) addToCacheIfAbsent(
	ctx context.Context, id appstatspb.StmtFingerprintID,
) bool {
	ss.cacheMu.Lock()
...
func (ss *StatementStore) Start(ctx context.Context, stopper *stop.Stopper) {
	...
	ss.memAcc = ss.mon.MakeConcurrentBoundAccount()
	...
	if err := stopper.RunAsyncTask(ctx,
		...
		log.Ops.Warningf(ctx,
			"failed to start statement store writer: %s", err)
		return
	}
}
...
func (ss *StatementStore) drainAndFlush(ctx context.Context) {
	...
	if ss.memAcc != nil {
		for i := range toFlush {
			flushSize += statementInfoSize(toFlush[i])
		}
	}
	...
	func() {
		ss.flushMu.Lock()
		defer ss.flushMu.Unlock()
		ss.memAcc.Shrink(ctx, flushSize)
	...
	for start := 0; start < len(entries); start += chunk {
		...
		if err := ss.batchInsertStatements(
			...
			if logEvery.ShouldLog() {
				log.Ops.Errorf(ctx,
					"error inserting statement fingerprints: %s", err)
			}
			for i := start; i < end; i++ {
...
// system.statements in a single INSERT ... ON CONFLICT DO UPDATE.
...
// statementInfoSize returns a rough estimate of the memory footprint
// of a StatementInfo entry in the pending buffer.
func statementInfoSize(info StatementInfo) int64 {
	// 64 bytes for struct overhead + string contents.
	return int64(64 + len(info.Fingerprint) + len(info.Database) + len(info.Summary))
}
