...
var crdbInternalClusterReplicationResolvedView = virtualSchemaView{
	schema: `
		CREATE VIEW crdb_internal.cluster_replication_spans AS WITH spans AS (
			SELECT 
				job_id, jsonb_array_elements(crdb_internal.pb_to_json('progress', value)->'streamIngest'->'checkpoint'->'resolvedSpans') AS s 
			FROM system.job_info 
			WHERE info_key = 'legacy_progress' AND crdb_internal.pb_to_json('progress', value)->'streamIngest' IS NOT NULL
			) SELECT 
				job_id, 
	...
	resultColumns: colinfo.ResultColumns{
		...
		{Name: "behind", Typ: types.Interval},
	},
}
