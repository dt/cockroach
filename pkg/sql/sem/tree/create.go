...
func (node *ColumnTableDef) Format(ctx *FmtCtx) {
	...
	if node.GeneratedIdentity.IsGeneratedAsIdentity {
		...
		if genSeqOpt := node.GeneratedIdentity.SeqOptions; len(genSeqOpt) > 0 {
			ctx.WriteString(" (")
...
func (node *SequenceOptions) ExtractIdentitySeqName() (*TableName, SequenceOptions) {
	var explicit *TableName
	remaining := make(SequenceOptions, 0, len(*node))
