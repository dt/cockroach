...
// allSpan is the conventional "everything" span used by the test
// driver — v1 has a single producer covering one logical span set.
...
// Tick paths only encode whole seconds (revlog-format.md §2), so
// tests place tick boundaries on whole-second multiples to get
// distinct marker paths.
...
func newTestDriver(t *testing.T, startHLC hlc.Timestamp) (*revlogjob.Driver, cloud.ExternalStorage) {
	t.Helper()
	es := newTestStorage(t)
...
// no tick is closed even if events are buffered. v1 deliberately
// drops in-flight buffers (TODO to flush on shutdown).
