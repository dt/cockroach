...
//
// See RFC §1 Concerns "PTS — v1 self-managed; future
// cluster-coordinated".
...
// ptsManager owns the v1 self-managed protected-timestamp lifecycle
// for the revlog writer job. It is constructed and held by the
// gateway-side TickManager wiring (see flow.go); per-node processors
// never see it.
//
// Lifecycle:
//
...
func (p *ptsManager) install(ctx context.Context) error {
	return p.job.NoTxn().Update(ctx, func(txn isql.Txn, md jobs.JobMetadata, ju *jobs.JobUpdater) error {
		...
		// If a PTS record is already recorded on the job, reuse it
		// and skip the (re-)Protect call. Persistent record IDs
		// across resumes are an inherited-from-resumption case for
		// post-v1 work; in v1 the only way the field is non-nil is
		// if a previous resumption installed it.
...
func (p *ptsManager) advance(ctx context.Context, frontier hlc.Timestamp) {
	...
	p.mu.Lock()
	// Only move recordTS forward — concurrent advance callers (none
	// in v1, but guard cheaply) might have raced past us.
