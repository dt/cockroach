...
//
// v1 simplifications, captured as TODOs below:
//
//   - Single producer covering all spans (no DistSQL split).
//   - Flush trigger is checkpoint-only; one file per tick, so
//     flush_order is always 0.
...
type tickBuffer struct {
	// TODO(revlog): track total byte size for size-triggered
...
// > startHLC.
//
// TODO(revlog): replan / dynamic re-partitioning. v1 takes a static
// span set on entry.
func NewProducer(
	es cloud.ExternalStorage,
	spans []roachpb.Span,
...
func (p *Producer) OnCheckpoint(
	ctx context.Context, sp roachpb.Span, ts hlc.Timestamp,
) error {
...
// flushTick writes any buffered events for tickEnd as a single SST
// data file and appends a FlushedFile entry to pendingFiles for the
// next Flush emission. Empty ticks (no buffered events) flush
// nothing — the coordinator's TickManager will still write a
// manifest for them when its aggregate frontier crosses the
// boundary.
//
// TODO(revlog): size/time-triggered intra-tick flushes. v1 flushes
// once per tick at the boundary, so flush_order is always 0.
func (p *Producer) flushTick(ctx context.Context, tickEnd hlc.Timestamp) error {
	...
	// already-delivered events after a reconnect (revlog-format.md
	// §7); (key, ts) uniquely identifies an MVCC revision, so any
	// such duplicates carry the same value and dropping them is
	// safe. Without this the SSTable writer would reject the second
	// occurrence ("keys must be added in strictly increasing order").
