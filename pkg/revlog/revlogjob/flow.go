...
// Run plans and executes the revlog DistSQL flow: one producer
// processor per SQL instance, all covering the (current) span set
// returned by resolveSpans, writing data files + producer-progress
// metadata. Metadata streams back to the gateway, where this
...
// v1 simplifications:
//
//   - All producers cover the same span set (no partitioning). A
//     real driver would partition spans across producers; that
//     requires multi-producer aggregation in TickManager (TODO).
//   - resolveSpans is invoked exactly once at startup; mid-job
//     coverage changes (RFC §1 "Span coverage tracking") are TODO.
...
func Run(
	...
	planCtx, sqlInstanceIDs, err := dsp.SetupAllNodesPlanning(
