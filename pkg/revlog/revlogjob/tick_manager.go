...
// aggregate frontier is closed: the manifest is written using
// whatever files have been collected (an empty list yields an
// empty-Files manifest, so coverage has no holes).
//
// In v1 there is one producer covering the same span set as the
// manager — so each producer Flush produces immediate manifest
// writes. Multi-producer support drops in by initializing the
// manager's frontier over the union of producer span sets and
// having each producer cover its assigned subset; the aggregate
// (= min across spans) advances only when every producer has
// reached the boundary, exactly as required.
type TickManager struct {
	...
	// afterFrontierAdvance, if non-nil, is invoked synchronously
	// from Flush each time the aggregate frontier advances. The
	// hook receives the new frontier value. v1 only consumer is the
	// PTS manager (see pts.go) which uses it to decide whether to
	// nudge the protected timestamp forward; failures inside the
	// hook are the hook's responsibility to log and swallow.
...
//
// TODO(revlog): in the multi-producer split, accept a producer ID
...
//
// TODO(revlog): schema poller hook on tick-close decisions (DDL
