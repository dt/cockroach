...
message Schema {
  ...
  // effective_from — every table, type, etc. covered. v1
  // captures the full snapshot on every entry; a delta
  // encoding (only changed descriptors, periodic full
  // snapshots) is a possible later optimization.
...
A future "sharded TickReader" — partitioning a single tick's
spans across multiple consumers for parallel restore /
catch-up — is left for when scale demands it. The above is the
v1 shape.
...
- **`Manifest.inline_tail`** is reserved for the v2
  small-files-coalescing change. v1 readers should accept and
  process it as if it were a virtual data file appended at
  the highest flushorder; v1 producers do not populate it.
